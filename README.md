...
- 設定ファイルの探索と優先度:
	- ランチャー実行時、デフォルトの設定基準フォルダは **`qgis_launcher.exe` と同じディレクトリ** になります。
	- 起動引数 `--settings_dir <パス>` を指定した場合は、そのディレクトリ内の `qgis_settings.json` を使用し、以降のパス解決もすべてそのディレクトリを基準に行います。
	- ※以前のバージョンで存在した `C:\qgis_launcher` への固定フォールバックや、JSON内部での絶対パス指定は廃止され、常に「EXEの配置場所（または引数指定場所）」ベースでポータブルに動作するようになりました。
