...
## QGIS ランチャー: 最近の変更とトラブルシュート

- **起動ログ**: ランチャーは QGIS の子プロセス出力を `qgis_child.log` に保存します（`qgis_launcher` 実行フォルダ内）。起動失敗やカスタマイズ適用状況の確認時はこのファイルを参照してください。
- **カスタマイズファイルの厳格ルール**: ランチャーは QGIS のメジャーバージョンに応じてカスタマイズファイル形式を厳格に切り替えます（フォールバック無し）。
  - QGIS4: 必ず `download/ini/<Role>.xml`（Customization XML）を `--customizationfile` として渡します。存在しない場合はカスタマイズは適用されません。
  - QGIS3: 必ず `download/ini/<Role>.ini`（INI フォーマット）を `--customizationfile` として渡します。存在しない場合はカスタマイズは適用されません。
  - ※以前の `_qgis4.xml` の特殊扱いや自動フォールバックは廃止されています。既存の XML/INI が正しく配置されていることを確認してください。
- **GPU / OpenGL のトラブルシュート**: QGIS が GPU 初期化で失敗する場合、実行前に環境変数を設定してソフトウェアレンダリングを試すと改善することがあります（環境に依存します）。例:
  - `QT_OPENGL=software`
  - `LIBGL_ALWAYS_SOFTWARE=1`
 これらを設定後、ランチャーを再起動して `qgis_child.log` を確認してください。
- **ログ確認の簡単手順**:
 1. `qgis_launcher` を起動（GUI または `cargo run --release`）。
 2. 問題発生時は `qgis_launcher/qgis_child.log` を開き、`GLES` / `OpenGL` / `ContextResult` 等のエラーメッセージを探してください。
- **今後の改善候補**:
  - 子プロセスの標準出力/標準エラーを直接パイプで捕捉する方式（Method B）の導入検討。
  - 生成する QGIS4 XML の詳細マッピング（ツールバー・UI 要素）を強化。
