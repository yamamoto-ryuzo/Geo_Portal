...
### 変更点
- GUI / CLI: 選択したプロジェクトファイルの `version` 属性に基づき、同じバージョンの QGIS 実行ファイルを自動選択する機能を追加（手動での変更は常に可能）。
- GUI: プロジェクト選択時に `Project File Version` を表示し、自動選択を行うようにしました。
- CLI: `--qgis_executable` 未指定時、プロジェクトのバージョンに合うインストール済み QGIS をフォールバックで自動選択します。

### 備考
- 自動選択はあくまで初期選択（フォールバック）であり、GUI のドロップダウンでユーザーがいつでも手動選択できます。
